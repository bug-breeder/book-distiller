...
- **Clarity lint (CLI, no AI):** `pnpm exec tsx src/cli.ts lint-lessons <slug>` → flags concepts that are missing a `#### Dig deeper` block (error), or whose Dig deeper is too short / uses vague filler (warning). `/tutor-prep` runs this after each chapter and re-dispatches once on an error.
...
- **Lesson notes teach, they don't summarize.** Each concept in a lesson note carries a concrete, visible `**Explanation:**` (a named quantity, a one-line example, or the core intuition — never a bare definition) plus a **required `#### Dig deeper`** block holding the **intuition** (why the mechanism/formula works) and a **worked example** (every number shown). The interactive generator renders Dig deeper as a collapsed-by-default `<DigDeeper>` disclosure under the concept. `src/lessons/clarity.ts` (`lint-lessons`) enforces this: missing Dig deeper is an error; short/filler is a warning. The clarity contract lives in `.claude/skills/tutor-prep/lesson-note-template.md` and `.claude/agents/book-analyst.md`, so every future book inherits it.
